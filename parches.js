...
/* ════════════════════════════════════════════════════════════════
   NEXUS PRO - MÓDULO BANCOS (versión final limpia)
   Se integra como tab #14 nativo del sistema
   ════════════════════════════════════════════════════════════════ */

(function () {
  'use strict';

  if (window.__NEXUS_BANCOS_FINAL__) return;
  window.__NEXUS_BANCOS_FINAL__ = true;
  ...
  function api() {
    try { return (typeof API !== 'undefined' && API) ? API : null; } catch(e) { return null; }
  }
  ...
  function auditSafe(action, detail) {
    try { if (typeof logAudit === 'function') logAudit(action, detail, 'Bancos'); } catch(e) {}
  }

  function sortBancos(arr) {
    return [...arr].sort((a, b) => {
  ...
  // ═══ LIMPIAR código viejo de versiones previas ═══
  function limpiarVersionesViejas() {
    [
      ...
      '#nxConfigBancoFallbackBarV64',
      '#mBancos'
    ].forEach(sel => document.querySelector(sel)?.remove());
  }

  // ═══ CARGAR bancos desde Supabase (con fallback) ═══
  async function loadBancos(force) {
    ...
    try {
      ...
      console.warn('Bancos: usando fallback', e);
    }
    bancos = sortBancos(FALLBACK.map(b => ({ ...b })));
    return bancos;
  }

  function duplicado(nombre, excluirId) {
    const n = norm(nombre);
    return bancos.some(b => norm(b.nombre) === n && String(b.id) !== String(excluirId || ''));
  }
  ...
  async function bancoEnUso(nombre) {
    ...
    try {
      ...
      return Array.isArray(data) && data.some(x => norm(x.banco) === norm(nombre));
    } catch(e) { return false; }
  }

  // ═══ ACTUALIZAR select de bancos en modal de cobros ═══
  async function actualizarSelectBancosCobros() {
    const select = document.getElementById('aBanco');
    if (!select) return;
    
    const lista = await loadBancos();
  ...
  window.nxActualizarSelectBancosCobros = actualizarSelectBancosCobros;

  // ═══ CREAR TAB #14 "BANCOS" en la barra de tabs ═══
  function crearTab() {
    const tabsBar = document.querySelector('#v-config .cfg-tabs-bar');
    if (!tabsBar) return false;
    
    if (document.getElementById('cfgTab14')) return true; // ya existe
    
    const btn = document.createElement('button');
    btn.id = 'cfgTab14';
    btn.className = 'cfg-tab';
    btn.setAttribute('onclick', 'switchCfgTab(14)');
    btn.innerHTML = '<i class="ti ti-building-bank"></i><span>Bancos</span>';
    
    tabsBar.appendChild(btn);
    return true;
  }

  // ═══ CREAR PANEL #14 (donde vive la gestión de bancos) ═══
  function crearPanel() {
    const vConfig = document.getElementById('v-config');
    if (!vConfig) return false;
    
    if (document.getElementById('cfgPanel14')) return true; // ya existe
    
    // Encontrar cfgPanel13 para insertar después
    const panel13 = document.getElementById('cfgPanel13');
    if (!panel13) return false;
    ...
    panel.innerHTML = `
      <div class="nc p5">
        <div class="ch">
          ...
          <button class="btn bsm bc1" type="button" onclick="window.nxRecargarBancos()">
        ...
        <div style="background:#eff6ff;border:1px solid #bfdbfe;border-radius:14px;padding:12px;margin:12px 0;color:#1e3a8a;font-size:12px;font-weight:600">
          ℹ️ Los bancos activos aparecerán en el selector de cobros. Los inactivos quedan ocultos pero se conservan en los abonos antiguos.
        </div>
        
        <div style="display:grid;grid-template-columns:1fr auto;gap:10px;align-items:end;margin-bottom:14px" class="nxBancosForm">
          <div class="fr" style="margin:0">
            <label>Agregar nuevo banco</label>
            <input type="text" id="nxNuevoBanco" placeholder="Ej: Banco López" style="width:100%">
          </div>
          <button class="btn bxl bc1" type="button" onclick="window.nxAgregarBanco()">
            <i class="ti ti-plus"></i> Agregar
          </button>
        </div>
        
        <div class="div"></div>
        <div id="nxListaBancos" style="margin-top:10px"></div>
      </div>
    `;
    
    // Insertar DESPUÉS del panel 13
    panel13.parentNode.insertBefore(panel, panel13.nextSibling);
    return true;
  }
  ...
  function renderLista() {
    ...
    if (!bancos.length) {
      cont.innerHTML = '<div style="padding:20px;text-align:center;color:#94a3b8;background:#f8fafc;border:1px dashed #cbd5e1;border-radius:12px">No hay bancos registrados.</div>';
      return;
    }
    
    cont.innerHTML = '<div style="display:grid;gap:8px">' + bancos.map(b => `
      <div class="nxBancoRow" style="display:flex;align-items:center;justify-content:space-between;gap:10px;padding:12px;border:1px solid #e2e8f0;border-radius:14px;background:#fff">
        <div style="flex:1;min-width:0">
          <div style="font-weight:700;color:#0f172a;font-size:14px">🏦 ${esc(b.nombre)}</div>
          <div style="font-size:11px;font-weight:700;margin-top:3px;color:${b.activo ? '#059669' : '#dc2626'}">
            ${b.activo ? '✓ ACTIVO' : '✗ INACTIVO'}
          </div>
        </div>
        <div class="nxBancoActions" style="display:flex;gap:5px;flex-wrap:wrap;justify-content:flex-end">
  ...
  // ═══ ACCIONES ═══
  window.nxAgregarBanco = async function() {
    const input = document.getElementById('nxNuevoBanco');
    ...
    try {
      await a.post('bancos', { nombre, activo: true });
      ...
      console.error(e);
      toastSafe('err', 'No se pudo agregar', 'Verifica permisos o duplicado');
    }
  };

  window.nxEditarBanco = async function(id) {
    await loadBancos(true);
    const banco = bancos.find(b => String(b.id) === String(id));
    if (!banco) return;
    ...
    try {
      await a.patch('bancos', 'id=eq.' + id, { nombre });
      ...
      console.error(e);
      toastSafe('err', 'No se pudo editar', 'Verifica permisos');
    }
  };

  window.nxToggleBanco = async function(id) {
    await loadBancos(true);
    const banco = bancos.find(b => String(b.id) === String(id));
    if (!banco) return;
    ...
    try {
      await a.patch('bancos', 'id=eq.' + id, { activo: nuevoEstado });
      toastSafe('ok', nuevoEstado ? 'Banco activado' : 'Banco desactivado', banco.nombre);
      auditSafe(nuevoEstado ? 'BANCO_ACTIVADO' : 'BANCO_DESACTIVADO', banco.nombre);
      await loadBancos(true);
      await actualizarSelectBancosCobros();
      renderLista();
    } catch(e) {
  ...
  window.nxEliminarBanco = async function(id) {
    await loadBancos(true);
    const banco = bancos.find(b => String(b.id) === String(id));
    if (!banco) return;
    
    if (!confirm('¿Eliminar este banco?\n\n' + banco.nombre + '\n\nSi tiene abonos asociados, se desactivará en lugar de eliminarse.')) return;
    ...
    if (usado) {
      toastSafe('err', 'Banco en uso', 'Tiene abonos asociados. Se desactivará.');
      try { await a.patch('bancos', 'id=eq.' + id, { activo: false }); } catch(e) {}
  ...
  window.nxRecargarBancos = async function() {
    await loadBancos(true);
    await actualizarSelectBancosCobros();
    renderLista();
    toastSafe('ok', 'Bancos recargados', '');
  };

  // ═══ HOOK para que cuando entren al tab 14, se carguen los bancos ═══
  function hookSwitchTab() {
    if (typeof window.switchCfgTab !== 'function') {
      setTimeout(hookSwitchTab, 500);
      return;
    }
    
    if (window.switchCfgTab.__nxBancosWrapped) return;
    
    const original = window.switchCfgTab;
    
    const wrapped = async function(n) {
      const result = original.apply(this, arguments);
      
      // Si entran al tab 14 (Bancos), cargar los datos
      if (n === 14) {
        await loadBancos(true);
        renderLista();
      }
    ...
    wrapped.__nxBancosWrapped = true;
    window.switchCfgTab = wrapped;
  }

  // ═══ HOOK para regAbono — agregar banco al guardar ═══
  function hookRegAbono() {
    if (typeof window.regAbono !== 'function') {
      setTimeout(hookRegAbono, 500);
      return;
    }
    
    if (window.regAbono.__nxBancosWrapped) return;
    
    const original = window.regAbono;
    
    const wrapped = async function() {
      // Validaciones que ya existían
      ...
      const requiereBanco = metodo === 'Transferencia' || metodo === 'Depósito' || metodo === 'Deposito';
      
      if (requiereBanco) {
        const banco = document.getElementById('aBanco')?.value || '';
        if (!banco) {
          toastSafe('err', 'Banco requerido', 'Selecciona el banco');
          return;
        }
        if (banco === 'Otros') {
      ...
      // Interceptar API.post para agregar banco
      const a = api();
      const postOrig = a?.post;
      
      if (a && postOrig && requiereBanco) {
        a.post = async function(tabla, datos) {
          ...
          return postOrig.call(this, tabla, datos);
        };
      }
  ...
  // ═══ ASEGURAR campo Banco en el modal de cobros ═══
  function asegurarCampoBanco() {
    const aMet = document.getElementById('aMet');
    if (!aMet) return;
    
    if (!document.getElementById('aBanco')) {
    ...
    // Listeners
    if (!aMet.__nxBancoLis) {
      aMet.__nxBancoLis = true;
      aMet.addEventListener('change', function() {
        ...
        const requiere = this.value === 'Transferencia' || this.value === 'Depósito' || this.value === 'Deposito';
    ...
    const aBanco = document.getElementById('aBanco');
    if (aBanco && !aBanco.__nxBancoLis) {
      aBanco.__nxBancoLis = true;
      aBanco.addEventListener('change', function() {
        const otros = document.getElementById('aBancoOtrosCont');
        const inp = document.getElementById('aBancoOtros');
        if (this.value === 'Otros') {
    ...
    actualizarSelectBancosCobros();
  }

  // ═══ CSS para móvil ═══
  function injectCSS() {
    if (document.getElementById('nx-bancos-final-css')) return;
    const style = document.createElement('style');
    style.id = 'nx-bancos-final-css';
    style.textContent = `
      #cfgPanel14 .nxBancosForm { width: 100% }
      @media(max-width:768px) {
        #cfgPanel14 .nxBancosForm {
          grid-template-columns: 1fr !important;
        }
        #cfgPanel14 .nxBancosForm .btn {
          width: 100% !important;
          min-height: 48px !important;
          justify-content: center !important;
        }
        #cfgPanel14 .nxBancoRow {
          display: grid !important;
          grid-template-columns: 1fr !important;
          gap: 10px !important;
        }
        #cfgPanel14 .nxBancoActions {
          display: grid !important;
          grid-template-columns: 1fr 1fr 1fr !important;
          width: 100% !important;
          gap: 6px !important;
        }
        #cfgPanel14 .nxBancoActions button {
          width: 100% !important;
