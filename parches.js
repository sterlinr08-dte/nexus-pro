...
/* ════════════════════════════════════════════════════════════════
   NEXUS PRO — COLUMNA CLIENTE EN SOLICITUDES / ENTREGAS
   
   PROBLEMA: La tabla "Entregas pendientes de confirmar" no muestra
   a qué cliente corresponde el cobro. Solo muestra agente, monto,
   método y referencia.
   
   SOLUCIÓN: Interceptar la función renderSolicitudes para agregar
   una columna CLIENTE que:
   - Si tiene cobro_id → busca el cliente en ST.clientes por ID
   - Si tiene nota con "Depósito directo de cliente X" → extrae X
   - Si no tiene ninguno → muestra "—" (entrega manual de agente)
   ════════════════════════════════════════════════════════════════ */

(function () {
  "use strict";
  if (window.__NEXUS_CLIENTE_SOLICITUDES__) return;
  window.__NEXUS_CLIENTE_SOLICITUDES__ = true;

  /* ─── Extraer nombre de cliente de una entrega ─── */
  function getNombreCliente(e) {
    // 1. Buscar por cobro_id (= cliente_id) en ST.clientes
    if (e.cobro_id) {
      const cli = (window.ST?.clientes || []).find(c => String(c.id) === String(e.cobro_id));
      if (cli?.nom) return cli.nom;
    }
    // 2. Extraer de la nota "Depósito directo de cliente NOMBRE"
    if (e.nota) {
      const match = e.nota.match(/[Dd]ep[oó]sito directo de cliente\s+(.+)/);
      if (match && match[1]) return match[1].trim();
    }
    // 3. Sin cliente identificado
    return null;
  }

  /* ─── Inyectar CSS para la columna cliente ─── */
  function inyectarCSS() {
    if (document.getElementById('nx-sol-cliente-css')) return;
    const s = document.createElement('style');
    s.id = 'nx-sol-cliente-css';
    s.textContent = `
      .nx-cli-badge {
        display: inline-flex;
        align-items: center;
      ...
      /* Ajustar ancho de columnas para dar espacio al cliente */
      .nxSL-table th:nth-child(2),
      .nxSL-table td:nth-child(2) {
        min-width: 90px;
      }
      .nx-sol-cli-col {
        min-width: 110px;
  ...
  /* ─── Modificar la tabla para agregar la columna CLIENTE ─── */
  function agregarColumnaCliente(view) {
    if (!view) return;

    // Encontrar TODAS las tablas de solicitudes (pendientes + confirmadas)
    const tablas = view.querySelectorAll('.nxSL-table');

    tablas.forEach(tabla => {
      // Evitar doble procesamiento
      if (tabla.dataset.nxClienteCol) return;
      tabla.dataset.nxClienteCol = '1';

      const thead = tabla.querySelector('thead tr');
      const tbody = tabla.querySelector('tbody');
      if (!thead || !tbody) return;

      // Agregar header CLIENTE después de AGENTE (columna 2)
      const thAgente = thead.children[1]; // AGENTE es la 2da columna
      if (!thAgente) return;

      const thCliente = document.createElement('th');
      thCliente.className = 'nx-sol-cli-col';
      thCliente.textContent = 'CLIENTE';
      thAgente.insertAdjacentElement('afterend', thCliente);

      // Para cada fila del tbody, agregar la celda de cliente
      const filas = tbody.querySelectorAll('tr');
      filas.forEach((tr, idx) => {
        // Obtener la entrega correspondiente de los datos
        // Las filas ya están renderizadas, necesitamos extraer el cliente
        // del onclick del botón (tiene el id de la entrega)
        const btnConf = tr.querySelector('[onclick*="nxConfirmarEntregaAdmin"]');
        const btnAnu  = tr.querySelector('[onclick*="nxAnularEntregaAdmin"]');
        const btnDep  = tr.querySelector('[onclick*="nxDepositarEntregaAdmin"]');

        let entradaId = null;
        const btn = btnConf || btnAnu || btnDep;
        if (btn) {
          const match = btn.getAttribute('onclick').match(/'([^']+)'/);
          if (match) entradaId = match[1];
        }

        let nomCliente = null;
        if (entradaId && window.__nxEntregasCache__) {
          const e = window.__nxEntregasCache__.find(x => String(x.id) === String(entradaId));
          if (e) nomCliente = getNombreCliente(e);
        }

        // Insertar celda después de la celda de AGENTE (columna 2 = índice 1)
        const tdAgente = tr.children[1];
        if (!tdAgente) return;

        const td = document.createElement('td');
        td.className = 'nx-sol-cli-col';
        if (nomCliente) {
          td.innerHTML = `<span class="nx-cli-badge"><i class="ti ti-user"></i>${nomCliente}</span>`;
        } else {
          td.innerHTML = `<span class="nx-cli-none">—</span>`;
        }
        tdAgente.insertAdjacentElement('afterend', td);
      });
    });
  }

  /* ─── Guardar cache de entregas cuando se cargan ─── */
  function hookCargarEntregas() {
    // Interceptar el fetch de entregas para cachear los datos crudos
    const origFetch = window.nxRefrescarSolicitudes;
    if (typeof origFetch !== 'function' || origFetch._nxCliHooked) return;

    window.nxRefrescarSolicitudes = async function() {
      const resultado = await origFetch.apply(this, arguments);
      // Después de refrescar, reaplicar columnas
      setTimeout(() => {
        inyectarCSS();
        const view = document.getElementById('v-solicitudes');
        if (view) agregarColumnaCliente(view);
      }, 200);
      return resultado;
    };
    window.nxRefrescarSolicitudes._nxCliHooked = true;
  }

  /* ─── Hook al render de solicitudes ─── */
  function hookRenderSolicitudes() {
    const orig = window.nxRenderSolicitudes;
    if (typeof orig !== 'function' || orig._nxCliHooked) return;

    window.nxRenderSolicitudes = async function() {
      const resultado = await orig.apply(this, arguments);
      setTimeout(() => {
        inyectarCSS();
        const view = document.getElementById('v-solicitudes');
        if (view) agregarColumnaCliente(view);
    ...
    window.nxRenderSolicitudes._nxCliHooked = true;
  }

  /* ─── Cachear entregas cuando el módulo las carga ─── */
  function hookGetEntregas() {
    // Interceptar la API para capturar los datos de entregas_admin
    const apiOrig = window.API;
    if (!apiOrig) return;

    const getOrig = apiOrig.get;
    if (!getOrig || getOrig._nxEntregasHook) return;

    apiOrig.get = async function(tabla, ...args) {
      const resultado = await getOrig.apply(this, [tabla, ...args]);
      if (tabla === 'entregas_admin' && Array.isArray(resultado)) {
        window.__nxEntregasCache__ = resultado;
      }
      return resultado;
    };
    apiOrig.get._nxEntregasHook = true;
  }
  ...
  function init() {
    ...
    const tryHook = function() {
      intentos++;
      hookRenderSolicitudes();
      hookCargarEntregas();

      // Si estamos en la vista de solicitudes ahora mismo, aplicar
      const view = document.getElementById('v-solicitudes');
      if (view && view.classList.contains('on')) {
        agregarColumnaCliente(view);
      }

      if (intentos < 20) setTimeout(tryHook, 500);
    };
    tryHook();

    console.log('✅ NEXUS: Columna cliente en Solicitudes activa');
  }
