...
- **Estado de migración:** AGUAPRO 100% migrado (6 pestañas, 21 pruebas Playwright en
  ...
  **POS 100% migrado (v47.8):** los 7 usos de `.nxLupaBox` (Vender, ventana "Buscar
  ...
  repasadas sin regresión (120 pruebas en total). **PENDIENTE** migrar el resto por
  tandas: Préstamos, Vehículos, Rifas (`.rfSearch` x2), Consultorio (`.nxMdSearch`), y el
  selector de CLIENTE del POS en cobro/factura (sigue siendo `<select>`, ver nota abajo
  sobre `ModalBusquedaBase`). Al migrar cada uno: conservar su lógica de filtro, solo cambiar el
  markup al componente — y repetir la verificación contra el CSS real, no asumir que
  "ya funcionó en AGUAPRO/Seguros" significa que funcionará igual en otro contenedor.
